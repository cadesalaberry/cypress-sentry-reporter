...
We want a consistent, lightweight, and review-friendly way to document architectural decisions for `cypress-sentry-reporter`. An ADR format helps capture context, options, and outcomes over time. The sibling project `vitest-sentry-reporter` uses MADR successfully, and this repository keeps the same conventions while remaining fully independent.
