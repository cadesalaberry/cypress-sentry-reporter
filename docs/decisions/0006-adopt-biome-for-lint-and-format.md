...
We want automated, enforced linting and formatting from the first commit, so
contributors get instant feedback instead of style nits in review. The tool
should:
...
- A `pre-commit` hook (via [lefthook](https://lefthook.dev)) runs Biome on staged
  files so issues are caught before CI.
