...
with Gitmoji (see `docs/COMMIT_CONVENTION.md`, ADR-0004). We want the version
to be derived automatically from the commit history from day one, so that:
...
- Configuration lives in `release-please-config.json` (release type `node`,
  ...
  `0.0.0`, so the first `feat:`-bearing release PR ships `0.1.0`.
...
release time when the publish job's frozen install rejects it (this exact
failure shipped a broken deploy in the sibling Vitest project). Enforcing
