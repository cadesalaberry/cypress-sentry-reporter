...
describe('utils', () => {
  ...
  it('runMetaTags exposes browser and testing-type tags', () => {
    expect(
      runMetaTags({
  ...
  it('runMetaTags tolerates missing metadata', () => {
    expect(runMetaTags(undefined)).toEqual({
  ...
  it('extras returns duration/suite path, cypress version and env snapshot', async () => {
    const detect = await getDetectProviderMock();
    (detect as any).mockReturnValue({
      name: 'github',
    ...
    const ex = extras(ctx, { cypressVersion: '12.17.4' });
    expect(ex).toEqual(
      expect.objectContaining({
        duration_ms: 50,
        suite_path: ['s'],
