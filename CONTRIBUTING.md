# Contributing to cypress-sentry-reporter

Thanks for taking the time to contribute! 🎉 This project is a small, focused
Cypress plugin that ships test failures and context to Sentry. Contributions of
...
- 🐛 **Report a bug** — open a [bug report](https://github.com/cadesalaberry/cypress-sentry-reporter/issues/new/choose).
- 💡 **Request a feature** — open a [feature request](https://github.com/cadesalaberry/cypress-sentry-reporter/issues/new/choose).
- 📝 **Improve the docs** — fix typos, clarify usage, add examples.
- 🧑‍💻 **Send code** — pick up an issue (look for the
  [`good first issue`](https://github.com/cadesalaberry/cypress-sentry-reporter/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22)
...
(see [ADR-0002](docs/decisions/0002-adopt-bun-as-package-manager.md)). The
...
#    Add CYPRESS_INSTALL_BINARY=0 if you don't need the e2e smoke test locally.
bun install
...
To exercise the reporter end-to-end against a real Cypress run (the
"dogfooding" equivalent — a fixture project under `e2e/` with one passing and
one deliberately failing spec, asserted in dry-run mode):
...
(see [ADR-0006](docs/decisions/0006-adopt-biome-for-lint-and-format.md)):
...
Gitmoji, e.g. `feat(plugin): ✨ add support for X`. The full convention and the
...
6. Open the PR; CI builds, runs unit tests, runs the Cypress e2e smoke test
   (against Cypress 12.17.4), lints, and validates the package. A maintainer
   will review.

## Sibling project: vitest-sentry-reporter

[`vitest-sentry-reporter`](https://github.com/cadesalaberry/vitest-sentry-reporter)
is this project's sibling for Vitest. The two repositories are **intentionally
unlinked** — shared modules (`ci-providers`, `actor-detectors`, `codeowners`,
the dry-run transport) were copied, not extracted into a common package, and
may drift. If you fix a bug in one of these modules, please check whether the
sibling repository needs the same fix and open a matching PR there.
